...
filenames <- list.files("../raw_data/", full.names = TRUE) #Get list of batch files
intensities.list <- map(filenames, read_csv) #Read files in to a list
...
targets.9003 <- read.xlsx("../phenotype/2015-9003-1_Sample Key.xlsx")
targets.9003$Slide.ID <- paste(targets.9003$general.array, targets.9003$genexstripe.controling.stripe, sep = "_")
targets.9006 <- read.xlsx("../phenotype/2015-9006_Sample Key.xlsx")
colnames(targets.9006)[8] <- "Slide.ID"
targets.9006$External.ID <- paste(targets.9006$External.ID, "I", sep = "")
targets.bind <- rbind(targets.9003, targets.9006)

cerebellum.targets <- read_csv("../phenotype/Cerebellum_targets.csv")
cerebellum.targets$Tissue <- "Cerebellum"
heart.targets <- read_csv("../phenotype/Heart_targets.csv")
heart.targets$Tissue <- "Heart"
drg.targets <- read_csv("../phenotype/DRG_targets.csv")
drg.targets$Tissue <- "DRG"
tissue.targets <- rbind(cerebellum.targets, heart.targets, drg.targets)

pheno.targets <- read.xlsx("../phenotype/GenExpr_Animal_Info.xlsx")
colnames(pheno.targets) %<>% str_replace("\\+", ".") %>% str_replace_all("\\-", ".")

targets.join <- merge(targets.bind, tissue.targets, by.x = "Slide.ID", by.y = "ArrayCode") %>% merge(pheno.targets, by.x = "External.ID", by.y = "ID.Organ") %>% select(External.ID, Slide.ID, Batch, Replicate, Gt, Sex, Treatment, TimePoint, RNA_RIN, Extraction.Batch, Body.weight.at.Diss, Age.at.diss, Tissue) %>% arrange(Tissue, TimePoint, Replicate, Gt, Treatment)
targets.join$External.ID %<>% str_replace("#", "")

lumi.raw <- lumiR("../raw_data/all_batches.tsv", lib.mapping = "lumiMouseIDMapping", checkDupId = TRUE, convertNuID = TRUE, QC = FALSE) #Read in the joined batches
frda.key <- match(targets.join$Slide.ID, colnames(lumi.raw)) #Match sample names in targets table to samples in lumi object
lumi.raw <- lumi.raw[,frda.key] #Subset lumi object to get rid of unrelated samples

pData(lumi.raw) <- targets.join #Add phenotype data
sampleNames(lumi.raw) <- lumi.raw$External.ID #Add sample names

lumi.vst <- lumiT(lumi.raw) #Perform variance stabilized transformation
saveRDS.gz(lumi.vst, file = "./save/lumi.vst.rda")

batch.colors <- c("black","navy","blue","red","orange","cyan","tan","purple","lightcyan","lightyellow","darkseagreen","brown","salmon","gold4","pink","green") #Assign batch colors
gen.boxplot("baseline_intensity_notnorm.jpg", lumi.vst,  batch.colors, "VST Transformed Intensity not normalized", "Intensity") #Make boxplot of transformed intensities

genotype.colors <- c("magenta", "green")
treatment.colors <- c("brown", "salmon", "yellow")
tissue.colors <- c("violet", "darkseagreen", "darkorange")
lumi.vst$Treatment %<>% toupper %>% factor
heatmap.bars <- gen.heatmapbars(batch.colors, genotype.colors, treatment.colors, tissue.colors, pData(lumi.vst)) %>% data.frame #create colors for heatmap bar
lumi.vst$Batch.Color <- as.character(heatmap.bars$Batch.Color) #add variable for batch color
lumi.vst$Genotype.Color <- as.character(heatmap.bars$Genotype.Color) #add variable for diagnosis color
lumi.vst$Treatment.Color <- as.character(heatmap.bars$Treatment.Color) #add variable for diagnosis color
lumi.vst$Tissue.Color <- as.character(heatmap.bars$Tissue.Color)
saveRDS.gz(lumi.vst, file = "./save/lumi.vst.rda")
...
symbols.lumi <- getSYMBOL(rownames(lumi.expr), 'lumiMouseAll.db') %>% is.na #Determine which remaining probes are unannotated
...
qcsum$External.ID <- rownames(qcsum) #Add sample name
qcsum$RIN <- lumi.expr.annot$RIN #Add RIN
qcsum$TimePoint <- lumi.expr.annot$TimePoint #Add sample number
...
gen.heatmap("baseline_heatmap_norm", lumi.expr.annot, "") #Make heatmap of sample clustering
...
gen.pca("baseline_mds_genotype", cm1, pData(lumi.expr.annot), genotype.colors, "Gt") #label PCs by status
gen.pca("baseline_mds_batch", cm1, phenoData(lumi.expr.annot), batch.colors, "Batch") #label PCs by batch
gen.pca("baseline_mds_treatment", cm1, phenoData(lumi.expr.annot), treatment.colors, "Treatment") #label PCs by batch
gen.pca("baseline_mds_tissue", cm1, phenoData(lumi.expr.annot), tissue.colors, "Tissue") #label PCs by batch
...
lumi.heart <- lumi.vst[,lumi.vst$Tissue == "Heart"]
lumi.heart.norm <- lumiN(lumi.heart, method = "rsn") #Normalize with robust spline regression
lumi.heart.qual <- lumiQ(lumi.heart.norm, detectionTh = 0.01) #The detection threshold can be adjusted here.  It is probably inadvisable to use anything larger than p < 0.05
lumi.heart.cutoff <- detectionCall(lumi.heart.qual) #Get the count of probes which passed the detection threshold per sample
lumi.heart.expr <- lumi.heart.qual[which(lumi.heart.cutoff > 0),] #Drop any probe where none of the samples passed detection threshold
symbols.lumi.heart <- getSYMBOL(rownames(lumi.heart.expr), 'lumiMouseAll.db') %>% is.na #Determine which remaining probes are unannotated
lumi.heart.annot <- lumi.heart.expr[!symbols.lumi.heart,] #Drop any probe which is not annotated
saveRDS.gz(lumi.heart.annot, file = "lumi.heart.annot.rda")

gen.boxplot("heart_intensity_norm.jpg", lumi.heart.annot, batch.colors, "RSN normalized signal intensity", "Intensity") #Make box plot of normalized intensities
gen.heatmap("heart_heatmap", lumi.heart.annot, "Clustering Based on Inter-Array Pearson Coefficient, Quantile normalized")
IAC.norm.heart <- gen.IACcluster("heart_IAC", exprs(lumi.heart.annot), "Outlier removed")
...
lumi.heart.mads <- apply(exprs(lumi.heart.annot), 1, mad)
lumi.heart.ordered <- order(lumi.heart.mads, decreasing = TRUE)
top500.dist.norm.heart <- gen.topgenes("heart_heatmap_500", lumi.heart.annot, "Clustering Based on the Top 500 Most Variable Genes", lumi.heart.ordered, 500)
top1000.dist.norm.heart <- gen.topgenes("heart_heatmap_1000", lumi.heart.annot, "Clustering Based on the Top 1000 Most Variable Genes", lumi.heart.ordered, 1000)

cm1.heart <- cmdscale(top1000.dist.norm.heart, eig = TRUE)
gen.pca("heart_mds_genotype", cm1.heart, phenoData(lumi.heart.annot), genotype.colors, "Gt")
gen.pca("heart_mds_batch", cm1.heart, phenoData(lumi.heart.annot), batch.colors, "Batch")
gen.pca("heart_mds_treatment", cm1.heart, phenoData(lumi.heart.annot), treatment.colors, "Treatment")

qcsum.heart <- lumi.heart.annot@QC$sampleSummary %>% t %>% data.frame
colnames(qcsum.heart) %<>% str_replace("\\.0\\.01\\.", "")
qcsum.heart$Sample.Name <- rownames(qcsum.heart)
qcsum.heart$RIN <- lumi.heart.annot$RIN
qcsum.heart$Sample.Num <- lumi.heart.annot$TimePoint
arrange(qcsum.heart, distance.to.sample.mean)

#Remove outliers
lumi.heart.remove <- grepl("F1277D|M1282D|M1263D", sampleNames(lumi.heart.annot))
#lumi.heart.rmout <- lumi.heart.annot[,!lumi.heart.remove]
lumi.heart.rmout <- lumi.heart.annot

gen.heatmap("heart_rmout_heatmap", lumi.heart.rmout, "Clustering Based on Inter-Array Pearson Coefficient, Quantile normalized")
IAC.norm.heart <- gen.IACcluster("heart_rmout_IAC", exprs(lumi.heart.rmout), "Outlier removed")
sd.raw.norm.heart <- gen.sdplot("heart_rmout_sd", IAC.norm.heart, "Outlier removed")
saveRDS.gz(IAC.norm.heart, file = "./save/IAC.norm.heart.rda")
saveRDS.gz(sd.raw.norm.heart, file = "./save/sd.raw.norm.heart.rda")

lumi.heart.mads <- apply(exprs(lumi.heart.rmout), 1, mad)
lumi.heart.ordered <- order(lumi.heart.mads, decreasing = TRUE)
top500.dist.norm.heart <- gen.topgenes("heart_rmout_heatmap_500", lumi.heart.rmout, "Clustering Based on the Top 500 Most Variable Genes", lumi.heart.ordered, 500)
top1000.dist.norm.heart <- gen.topgenes("heart_rmout_heatmap_1000", lumi.heart.rmout, "Clustering Based on the Top 1000 Most Variable Genes", lumi.heart.ordered, 1000)

cm1.heart <- cmdscale(top1000.dist.norm.heart, eig = TRUE)
gen.pca("heart_rmout_mds_genotype", cm1.heart, phenoData(lumi.heart.rmout), genotype.colors, "Gt")
gen.pca("heart_rmout_mds_batch", cm1.heart, phenoData(lumi.heart.rmout), batch.colors, "Batch")
gen.pca("heart_rmout_mds_treatment", cm1.heart, phenoData(lumi.heart.rmout), treatment.colors, "Treatment")

#Use ComBat for batch effect correction
lumi.rm.onebatch <- lumi.heart.rmout[,lumi.heart.rmout$Batch != 8]
lumi.rm.onebatch$Gt.Treatment <- paste(lumi.rm.onebatch$Gt, lumi.rm.onebatch$Treatment, sep = ".")

model.gt.treatment <- model.matrix( ~ 0 + factor(lumi.rm.onebatch$Gt.Treatment) ) #Make dummy variables out of Sex
colnames(model.gt.treatment) <- c("Tg.DOX", "Tg.DOXR", "Tg.ND", "WT.DOX") model.gt.treatment.reduce <- model.gt.treatment[,-4] #Remove one column (like done with Status)

model.sex <- model.matrix( ~ 0 + factor(lumi.rm.onebatch$Sex) ) #Make dummy variables out of Sex
colnames(model.sex) <- c("Female", "Male")
model.sex.reduce <- model.sex[,-1] #Remove one column (like done with Status)

model.combat <- data.frame(model.gt.treatment.reduce, Male = model.sex.reduce, Weight = as.numeric(lumi.rm.onebatch$Body.weight.at.Diss), RIN = lumi.rm.onebatch$RNA_RIN, Age = lumi.rm.onebatch$Age.at.diss)  #Assemble covariate matrix with both categorical and continuous covariates
model.combat$RIN <- model.combat$RIN - min(model.combat$RIN)

expr.combat <- ComBat(dat = exprs(lumi.rm.onebatch), batch = factor(lumi.rm.onebatch$Batch), mod = model.combat) #Run ComBat
lumi.combat <- lumi.rm.onebatch #Create a new lumi object as a copy of lumi.rmreps.annot
...
model.all <- model.matrix( ~ Sex + Batch + Body.weight.at.Diss + RNA_RIN + Age.at.diss + Gt.Treatment + TimePoint, pData(lumi.collapse))
fit.all <- lmFit(lumi.collapse, design = model.all) %>% eBayes
...
decide.final <- gen.decide(c("none", 0.001), fit.object5, TRUE) %>% melt(id.vars = c("Test", "Num", "Direction")) #Compute signifance cutoff p < 0.001, no FDR adjustment
...
fit.selection <- gen.tables(de.object, lumi.time5, ratio.exp5, "pLess001") #create differential expression table for unadjusted fit
...
writeLines(de.doxnd.pco.up, "./doxnd.pco.up.txt")

drg.mouse.doxnd.up <- readRDS.gz("../baseline_drg/save/de.mouse.doxnd.up.rda")
drg.mouse.doxnd.down <- readRDS.gz("../baseline_drg/save/de.mouse.doxnd.down.rda")
drg.mouse.tgwt.up <- readRDS.gz("../baseline_drg/save/de.mouse.tgwt.up.rda")
drg.mouse.tgwt.down <- readRDS.gz("../baseline_drg/save/de.mouse.tgwt.down.rda")

cerebellum.mouse.doxnd.up <- readRDS.gz("../baseline_cerebellum/save/de.mouse.doxnd.up.rda")
cerebellum.mouse.doxnd.down <- readRDS.gz("../baseline_cerebellum/save/de.mouse.doxnd.down.rda")
cerebellum.mouse.tgwt.up <- readRDS.gz("../baseline_cerebellum/save/de.mouse.tgwt.up.rda")
cerebellum.mouse.tgwt.down <- readRDS.gz("../baseline_cerebellum/save/de.mouse.tgwt.down.rda")

de.doxnd.drg.up <- de.mouse.doxnd.up[de.mouse.doxnd.up %in% drg.mouse.doxnd.up]
de.doxnd.drg.down <- de.mouse.doxnd.down[de.mouse.doxnd.down %in% drg.mouse.doxnd.down]
de.tgwt.drg.up <- de.mouse.tgwt.up[de.mouse.tgwt.up %in% drg.mouse.tgwt.up]
de.tgwt.drg.down <- de.mouse.tgwt.down[de.mouse.tgwt.down %in% drg.mouse.tgwt.down]

de.doxnd.cerebellum.up <- de.mouse.doxnd.up[de.mouse.doxnd.up %in% cerebellum.mouse.doxnd.up]
de.doxnd.cerebellum.down <- de.mouse.doxnd.down[de.mouse.doxnd.down %in% cerebellum.mouse.doxnd.down]
de.tgwt.cerebellum.up <- de.mouse.tgwt.up[de.mouse.tgwt.up %in% cerebellum.mouse.tgwt.up]
de.tgwt.cerebellum.down <- de.mouse.tgwt.down[de.mouse.tgwt.down %in% cerebellum.mouse.tgwt.down]

drg.doxnd.cerebellum.up <- drg.mouse.doxnd.up[drg.mouse.doxnd.up %in% cerebellum.mouse.doxnd.up]
drg.doxnd.cerebellum.down <- drg.mouse.doxnd.down[drg.mouse.doxnd.down %in% cerebellum.mouse.doxnd.down]
drg.tgwt.cerebellum.up <- drg.mouse.tgwt.up[drg.mouse.tgwt.up %in% cerebellum.mouse.tgwt.up]
drg.tgwt.cerebellum.down <- drg.mouse.tgwt.down[drg.mouse.tgwt.down %in% cerebellum.mouse.tgwt.down]

hyper.doxnd.drg.up <- phyper(length(de.doxnd.drg.up), cutoff, length(mouse.overlap) - cutoff, cutoff, lower.tail = F)
hyper.doxnd.drg.down <- phyper(length(de.doxnd.drg.down), cutoff, length(mouse.overlap) - cutoff, cutoff, lower.tail = F)
hyper.doxnd.cerebellum.up <- phyper(length(de.doxnd.cerebellum.up), cutoff, length(mouse.overlap) - cutoff, cutoff, lower.tail = F)
hyper.doxnd.cerebellum.down <- phyper(length(de.doxnd.cerebellum.down), cutoff, length(mouse.overlap) - cutoff, cutoff, lower.tail = F)

hyper.tgwt.drg.up <- phyper(length(de.tgwt.drg.up), cutoff, length(mouse.overlap) - cutoff, cutoff, lower.tail = F)
hyper.tgwt.drg.down <- phyper(length(de.tgwt.drg.down), cutoff, length(mouse.overlap) - cutoff, cutoff, lower.tail = F)
hyper.tgwt.cerebellum.up <- phyper(length(de.tgwt.cerebellum.up), cutoff, length(mouse.overlap) - cutoff, cutoff, lower.tail = F)
hyper.tgwt.cerebellum.down <- phyper(length(de.tgwt.cerebellum.down), cutoff, length(mouse.overlap) - cutoff, cutoff, lower.tail = F)

hyper.drg.doxnd.cerebellum.up <- phyper(length(drg.doxnd.cerebellum.up), cutoff, length(mouse.overlap) - cutoff, cutoff, lower.tail = F)
hyper.drg.doxnd.cerebellum.down <- phyper(length(drg.doxnd.cerebellum.down), cutoff, length(mouse.overlap) - cutoff, cutoff, lower.tail = F)
hyper.drg.tgwt.cerebellum.up <- phyper(length(drg.tgwt.cerebellum.up), cutoff, length(mouse.overlap) - cutoff, cutoff, lower.tail = F)
hyper.drg.tgwt.cerebellum.down <- phyper(length(drg.tgwt.cerebellum.down), cutoff, length(mouse.overlap) - cutoff, cutoff, lower.tail = F)

tissue.list <- c(hyper.doxnd.drg.up, hyper.doxnd.drg.down, hyper.doxnd.cerebellum.up, hyper.doxnd.cerebellum.down, hyper.tgwt.drg.up, hyper.tgwt.drg.down, hyper.tgwt.cerebellum.up, hyper.tgwt.cerebellum.down, hyper.drg.doxnd.cerebellum.up, hyper.drg.doxnd.cerebellum.down, hyper.drg.tgwt.cerebellum.up, hyper.drg.tgwt.cerebellum.down)
