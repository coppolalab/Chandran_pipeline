...
#DRG
lumi.drg <- lumi.vst[,lumi.vst$Tissue == "DRG"]
lumi.drg.norm <- lumiN(lumi.drg, method = "rsn") #Normalize with robust spline regression
lumi.drg.qual <- lumiQ(lumi.drg.norm, detectionTh = 0.01) #The detection threshold can be adjusted here.  It is probably inadvisable to use anything larger than p < 0.05
lumi.drg.cutoff <- detectionCall(lumi.drg.qual) #Get the count of probes which passed the detection threshold per sample
lumi.drg.expr <- lumi.drg.qual[which(lumi.drg.cutoff > 0),] #Drop any probe where none of the samples passed detection threshold
symbols.lumi.drg <- getSYMBOL(rownames(lumi.drg.expr), 'lumiMouseAll.db') %>% is.na #Determine which remaining probes are unannotated
lumi.drg.annot <- lumi.drg.expr[!symbols.lumi.drg,] #Drop any probe which is not annotated
saveRDS.gz(lumi.drg.annot, file = "lumi.drg.annot.rda")
...
gen.boxplot("drg_intensity_norm.jpg", lumi.drg.annot, batch.colors, "RSN normalized signal intensity", "Intensity") #Make box plot of normalized intensities
gen.heatmap("drg_heatmap", lumi.drg.annot, "Clustering Based on Inter-Array Pearson Coefficient, Quantile normalized")
IAC.norm.drg <- gen.IACcluster("drg_IAC", exprs(lumi.drg.annot), "Outlier removed")
sd.raw.norm.drg <- gen.sdplot("drg_sd", IAC.norm.drg, "Outlier removed")
saveRDS.gz(IAC.norm.drg, file = "./save/IAC.norm.drg.rda")
saveRDS.gz(sd.raw.norm.drg, file = "./save/sd.raw.norm.drg.rda")

lumi.drg.mads <- apply(exprs(lumi.drg.annot), 1, mad)
lumi.drg.ordered <- order(lumi.drg.mads, decreasing = TRUE)
top500.dist.norm.drg <- gen.topgenes("drg_heatmap_500", lumi.drg.annot, "Clustering Based on the Top 500 Most Variable Genes", lumi.drg.ordered, 500)
top1000.dist.norm.drg <- gen.topgenes("drg_heatmap_1000", lumi.drg.annot, "Clustering Based on the Top 1000 Most Variable Genes", lumi.drg.ordered, 1000)

cm1.drg <- cmdscale(top1000.dist.norm.drg, eig = TRUE)
gen.pca("drg_mds_genotype", cm1.drg, phenoData(lumi.drg.annot), genotype.colors, "Gt")
gen.pca("drg_mds_batch", cm1.drg, phenoData(lumi.drg.annot), batch.colors, "Batch")
gen.pca("drg_mds_treatment", cm1.drg, phenoData(lumi.drg.annot), treatment.colors, "Treatment")

qcsum.drg <- lumi.drg.annot@QC$sampleSummary %>% t %>% data.frame
colnames(qcsum.drg) %<>% str_replace("\\.0\\.01\\.", "")
qcsum.drg$Sample.Name <- rownames(qcsum.drg)
qcsum.drg$RIN <- lumi.drg.annot$RIN
qcsum.drg$Sample.Num <- lumi.drg.annot$TimePoint
arrange(qcsum.drg, distance.to.sample.mean)

lumi.drg.remove <- grepl("F1383I", sampleNames(lumi.drg.annot))
lumi.drg.rmout <- lumi.drg.annot[,!lumi.drg.remove]

gen.heatmap("drg_rmout_heatmap", lumi.drg.rmout, "Clustering Based on Inter-Array Pearson Coefficient, Quantile normalized")
IAC.norm.drg <- gen.IACcluster("drg_rmout_IAC", exprs(lumi.drg.rmout), "Outlier removed")
sd.raw.norm.drg <- gen.sdplot("drg_rmout_sd", IAC.norm.drg, "Outlier removed")
saveRDS.gz(IAC.norm.drg, file = "./save/IAC.norm.drg.rda")
saveRDS.gz(sd.raw.norm.drg, file = "./save/sd.raw.norm.drg.rda")

lumi.drg.mads <- apply(exprs(lumi.drg.rmout), 1, mad)
lumi.drg.ordered <- order(lumi.drg.mads, decreasing = TRUE)
top500.dist.norm.drg <- gen.topgenes("drg_rmout_heatmap_500", lumi.drg.rmout, "Clustering Based on the Top 500 Most Variable Genes", lumi.drg.ordered, 500)
top1000.dist.norm.drg <- gen.topgenes("drg_rmout_heatmap_1000", lumi.drg.rmout, "Clustering Based on the Top 1000 Most Variable Genes", lumi.drg.ordered, 1000)

cm1.drg <- cmdscale(top1000.dist.norm.drg, eig = TRUE)
gen.pca("drg_rmout_mds_genotype", cm1.drg, phenoData(lumi.drg.rmout), genotype.colors, "Gt")
gen.pca("drg_rmout_mds_batch", cm1.drg, phenoData(lumi.drg.rmout), batch.colors, "Batch")
gen.pca("drg_rmout_mds_treatment", cm1.drg, phenoData(lumi.drg.rmout), treatment.colors, "Treatment")

#Use ComBat for batch effect correction
lumi.drg.rmout$Gt.Treatment <- paste(lumi.drg.rmout$Gt, lumi.drg.rmout$Treatment, sep = ".")

model.gt.treatment <- model.matrix( ~ 0 + factor(lumi.drg.rmout$Gt.Treatment) ) #Make dummy variables out of Sex
...
model.sex <- model.matrix( ~ 0 + factor(lumi.drg.rmout$Sex) ) #Make dummy variables out of Sex
colnames(model.sex) <- c("Female", "Male")
model.sex.reduce <- model.sex[,-1] #Remove one column (like done with Status)

model.combat <- data.frame(model.gt.treatment.reduce, Male = model.sex.reduce, Weight = as.numeric(lumi.drg.rmout$Body.weight.at.Diss), RIN = lumi.drg.rmout$RNA_RIN, Age = lumi.drg.rmout$Age.at.diss)  #Assemble covariate matrix with both categorical and continuous covariates
model.combat$RIN <- model.combat$RIN - min(model.combat$RIN)

expr.combat <- ComBat(dat = exprs(lumi.drg.rmout), batch = factor(lumi.drg.rmout$Batch), mod = model.combat) #Run ComBat
lumi.combat <- lumi.drg.rmout #Create a new lumi object as a copy of lumi.rmreps.annot
